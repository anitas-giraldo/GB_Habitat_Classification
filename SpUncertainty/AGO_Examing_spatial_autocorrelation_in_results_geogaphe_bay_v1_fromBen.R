...
dat <- shapefile(paste(w.dir, ("BRUVS_validation_Sent2_B321_Depth1819_sharyn.shp"), sep ='/'))
## add projection/check 
proj4string(dat) <- CRS('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0 ')
dat
str(dat)
plot(dat, pch =21, bg = dat$classvalue)
plot(dat, pch =21, bg = dat$classval_1)
dat$classvalue
dat$Classval_1
...
dat$correct <- as.numeric(ifelse(dat$classvalue == dat$Classval_1,1,0))
