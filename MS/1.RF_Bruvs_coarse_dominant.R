...
# remove point that fall outside CMR ---
names(df)
dfs <- df
coordinates(dfs) <- ~longitude + latitude
points(dfs)
head(dfs)
proj4string(dfs) <- proj4string(p)

dfs2 <- raster::extract(p$depth, dfs, df = TRUE)
head(dfs2)

dfs2 <- cbind(dfs, dfs2)
head(dfs2)

df2 <- dfs2[!is.na(dfs2$depth.1),]
str(df2)
...
# these classes only have two observation as dominant in the whole data set --

df2 <- df2[df2$Class != "Sponges",]
df2 <- df2[df2$Class != "Consolidated",]
df2 <- df2[df2$Class != "Macroalgae",] # only 4 obs
df2 <- droplevels(df2)
summary(df2)
levels(df2$Class)
...
preds.df <- df2[,-1]
head(preds.df)
preds.pca <- prcomp(preds.df, scale = TRUE)
# Visualize eigenvalues (scree plot). Show the percentage of variances explained by each principal component.
fviz_eig(preds.pca)

# Graph of individuals. Individuals with a similar profile are grouped together
fviz_pca_ind(preds.pca,
...
fviz_pca_var(preds.pca,
             col.var = "contrib", # Color by contributions to the PC
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE     # Avoid text overlapping
)

class <- as.factor(df2$Class)
fviz_pca_ind(preds.pca,
             col.ind = class, # color by groups
             palette = c("#00AFBB",  "#FC4E07", "#E7B800", "#FF6600", "#3399FF", "#CC33FF"),
...
## Change the name of the classes for models ----
#train <- train %>% mutate(Class = car::recode(Class, "'Unconsolidated'='Unvegetated';'Seagrasses' = 'Seagrass'; c('Turf.algae','Macroalgae')='Algae'"))
#summary(train)

#test1 <- test %>% mutate(Class = car::recode(Class, "'Unconsolidated'='Unvegetated';'Seagrasses' = 'Seagrass'; c('Turf.algae','Macroalgae')='Algae'"))
#summary(test1)
...
oob.error.data <- data.frame(
  ...
  Type=rep(c("OOB", "Seagrasses", "Turf.algae", "Unconsolidated"), each=nrow(model$err.rate)),
...
# Validation ----
## I think this validation is wrong - CHECK Ben's script
...
plot(rf.def)
summary(rf.def) 
rf.def$varselect.pred # [1] 1 4 6
head(TrainData) # depth, aspect4, tpi
...
distance.matrix <- as.dist(1-model2$proximity)

distance.matrix <- dist(model$proximity, method = "euclidean")
