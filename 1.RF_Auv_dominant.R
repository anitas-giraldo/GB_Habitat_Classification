...
model6 <- randomForest(Class ~ ., data=train %>% mutate(Class = car::recode(Class, "c('Unconsolidated', 'Consolidated')='Unvegetated';'Seagrasses' = 'Seagrass'; c('Turf.algae','Macroalgae')='Algae'")) %>%
                         select(Class, depth, slope4, aspect4), 
                       ntree=501, proximity=TRUE, mtry = 3, importance=TRUE)
model6 #  OOB = 23.96%
model6$importance
varImpPlot(model6)
...
lp2
trellis.device(device ="png", filename = paste(p.dir, "AUV-fine.png", sep='/'), width = 1000, height = 670, res = 200)
print(lp2)
dev.off()
